...
void my_topk (int *p_n, float *topk, float *y)
{
  ...
  for (int c=0; c<n; c+=8) {
    ...
    // topk, it contains undefined value so protect against it
    indices = _mm256_set1_epi32(-1);

    k0 = _mm256_cmp_ps_mask(y0, curr_topk, _CMP_GT_OS);
    if (k0) {
      indices =  _mm256_mask_set1_epi32(indices, 0x1, _bit_scan_reverse(k0));
    }

    k1 = _mm256_cmp_ps_mask(y1, curr_topk, _CMP_GT_OS);
    if (k1) {
      indices =  _mm256_mask_set1_epi32(indices, 0x2, _bit_scan_reverse(k1));
    }

    k2 = _mm256_cmp_ps_mask(y2, curr_topk, _CMP_GT_OS);
    if (k2) {
      indices =  _mm256_mask_set1_epi32(indices, 0x4, _bit_scan_reverse(k2));
    }

    k3 = _mm256_cmp_ps_mask(y3, curr_topk, _CMP_GT_OS);
    if (k3) {
      indices =  _mm256_mask_set1_epi32(indices, 0x8, _bit_scan_reverse(k3));
    }

    k4 = _mm256_cmp_ps_mask(y4, curr_topk, _CMP_GT_OS);
    if (k4) {
      indices =  _mm256_mask_set1_epi32(indices, 0x10, _bit_scan_reverse(k4));
    }

    k5 = _mm256_cmp_ps_mask(y5, curr_topk, _CMP_GT_OS);
    if (k5) {
      indices =  _mm256_mask_set1_epi32(indices, 0x20, _bit_scan_reverse(k5));
    }

    k6 = _mm256_cmp_ps_mask(y6, curr_topk, _CMP_GT_OS);
    if (k6) {
      indices =  _mm256_mask_set1_epi32(indices, 0x40, _bit_scan_reverse(k6));
    }

    k7 = _mm256_cmp_ps_mask(y7, curr_topk, _CMP_GT_OS);
    if (k7) {
      indices =  _mm256_mask_set1_epi32(indices, 0x80, _bit_scan_reverse(k7));
    }
