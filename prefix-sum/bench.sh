...
taskset -c 4 numactl -m0 ./ref.out $strt $end $step >> icc-ref.log
taskset -c 4 numactl -m0 ./omp.out $strt $end $step >> icc-omp.log
...
awk '/perf/ {print $(NF)}' icc-ref.log | tr -d , > c6.dat
awk '/perf/ {print $(NF)}' icc-omp.log | tr -d , > c7.dat
